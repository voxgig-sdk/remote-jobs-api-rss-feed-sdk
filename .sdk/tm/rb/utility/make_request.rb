...
module RemoteJobsApiRssFeedUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = RemoteJobsApiRssFeedResponse.new({})
    ...
    if fetch_err
      ...
      response = RemoteJobsApiRssFeedResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = RemoteJobsApiRssFeedResponse.new(fetched)
    else
