...
// RemoteJobsApiRssFeed SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class RemoteJobsApiRssFeedMakePoint
{
    public static function call(RemoteJobsApiRssFeedContext $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof RemoteJobsApiRssFeedError) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $point = $p;
                $select_def = RemoteJobsApiRssFeedHelpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = RemoteJobsApiRssFeedHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
