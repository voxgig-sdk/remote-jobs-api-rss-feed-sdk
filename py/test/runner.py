# RemoteJobsApiRssFeed SDK test runner
...
class RemoteJobsApiRssFeedTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            RemoteJobsApiRssFeedTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = RemoteJobsApiRssFeedTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = RemoteJobsApiRssFeedTestRunner.getenv("REMOTE_JOBS_API_RSS_FEED_TEST_LIVE")
        override = RemoteJobsApiRssFeedTestRunner.getenv("REMOTE_JOBS_API_RSS_FEED_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = RemoteJobsApiRssFeedTestRunner.getenv(key)
        ...
        explain = RemoteJobsApiRssFeedTestRunner.getenv("REMOTE_JOBS_API_RSS_FEED_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if RemoteJobsApiRssFeedTestRunner._test_control is not None:
            return RemoteJobsApiRssFeedTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                RemoteJobsApiRssFeedTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            RemoteJobsApiRssFeedTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = RemoteJobsApiRssFeedTestRunner.load_test_control()
...
def load_env_local():
    RemoteJobsApiRssFeedTestRunner.load_env_local()


def env_override(m):
    return RemoteJobsApiRssFeedTestRunner.env_override(m)


def entity_data(v):
    return RemoteJobsApiRssFeedTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return RemoteJobsApiRssFeedTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return RemoteJobsApiRssFeedTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return RemoteJobsApiRssFeedTestRunner.load_test_control()


def live_client_options():
    return RemoteJobsApiRssFeedTestRunner.live_client_options()


def live_delay_ms():
    return RemoteJobsApiRssFeedTestRunner.live_delay_ms()
