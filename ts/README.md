# RemoteJobsApiRssFeed TypeScript SDK



The TypeScript SDK for the RemoteJobsApiRssFeed API — a type-safe, entity-oriented client with full async/await support.

The API is exposed as capitalised, semantic **Entities** — e.g.
`client.RemoteJob()` — each with a small set of operations (`list`)
...
```ts
new RemoteJobsApiRssFeedSDK(options?: {
  base?: string
  prefix?: string
  suffix?: string
...
| `RemoteJob(data?)` | `RemoteJobEntity` | Create a RemoteJob entity instance. |
| `tester(testopts?, sdkopts?)` | `RemoteJobsApiRssFeedSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `RemoteJobsApiRssFeedSDK.test(testopts?, sdkopts?)` | `RemoteJobsApiRssFeedSDK` | Create a test-mode client. |
...
| `client` | `client(): RemoteJobsApiRssFeedSDK` | Return the parent SDK client. |
...
| `salaryMax` | `number | string` | Max salary (if applicable) |
| `salaryMin` | `number | string` | Min salary (if applicable) |
...
├── src/
│   ├── RemoteJobsApiRssFeedSDK.ts        # Main SDK class
...
```ts
const remotejob = client.RemoteJob()
await remotejob.list()
