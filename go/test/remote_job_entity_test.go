...
func TestRemoteJobEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.RemoteJob(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.RemoteJob(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		remoteJobRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.remote_job")))
...
func remote_jobBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("REMOTE_JOBS_API_RSS_FEED_TEST_REMOTE_JOB_ENTID")
	...
	idmapResolved := core.ToMapAny(env["REMOTE_JOBS_API_RSS_FEED_TEST_REMOTE_JOB_ENTID"])
