# remote-jobs-api-rss-feed-mcp

[MCP](https://modelcontextprotocol.io) server exposing the RemoteJobsApiRssFeed SDK as
two agent tools — `remote-jobs-api-rss-feed_list` and `remote-jobs-api-rss-feed_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/remote-jobs-api-rss-feed-mcp)
make build

# 2. Provide credentials via the environment
export REMOTE_JOBS_API_RSS_FEED_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user remote-jobs-api-rss-feed \
  -- /absolute/path/to/remote-jobs-api-rss-feed-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./remote-jobs-api-rss-feed-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// remote-jobs-api-rss-feed_list: first page of records
{ "entity": "remote_job" }
{ "entity": "remote_job", "query": { } }

// remote-jobs-api-rss-feed_load: one record by id
{ "entity": "remote_job", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/remote-jobs-api-rss-feed-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user remote-jobs-api-rss-feed \
     -- "$PWD"/dist/*/remote-jobs-api-rss-feed-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `remote-jobs-api-rss-feed_list` and `remote-jobs-api-rss-feed_load` tools now appear
   in new sessions. Ask the agent to *"list remote_job using remote-jobs-api-rss-feed"*
   and it calls `remote-jobs-api-rss-feed_list` with `{"entity":"remote_job"}`.
...
```sh
export REMOTE_JOBS_API_RSS_FEED_APIKEY=sk_live_xxx            # API key
export REMOTE_JOBS_API_RSS_FEED_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `remote-jobs-api-rss-feed_list` tool
...
### Call the `remote-jobs-api-rss-feed_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `remote-jobs-api-rss-feed_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `remote-jobs-api-rss-feed_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `REMOTE_JOBS_API_RSS_FEED_APIKEY` | API key sent with every request. |
| `REMOTE_JOBS_API_RSS_FEED_BASE` | Optional override of the API base URL. |
...
```sh
./remote-jobs-api-rss-feed-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"remote-jobs-api-rss-feed_load","arguments":{"entity":"remote_job","query":{"id":1}}}}'
```
