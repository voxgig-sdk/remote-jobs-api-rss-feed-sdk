...
(0, node_test_1.describe)('RemoteJobEntity', async () => {
    // Per-test live pacing. Delay is read from sdk-test-control.json's
    // `test.live.delayMs`; only sleeps when REMOTE_JOBS_API_RSS_FEED_TEST_LIVE=TRUE.
    (0, node_test_1.afterEach)((0, utility_1.liveDelay)('REMOTE_JOBS_API_RSS_FEED_TEST_LIVE'));
    (0, node_test_1.test)('instance', async () => {
        const testsdk = __1.RemoteJobsApiRssFeedSDK.test();
...
function basicSetup(extra) {
    ...
    const entityDataFile = node_path_1.default.resolve(__dirname, '../../../../.sdk/test/entity/remote_job/RemoteJobTestData.json');
    // TODO: file ready util needed?
    const entityDataSource = Fs.readFileSync(entityDataFile).toString('utf8');
    // TODO: need a xlang JSON parse utility in voxgig/struct with better error msgs
    const entityData = JSON.parse(entityDataSource);
    options.entity = entityData.existing;
    let client = __1.RemoteJobsApiRssFeedSDK.test(options, extra);
    const struct = client.utility().struct;
    const merge = struct.merge;
    const transform = struct.transform;
    let idmap = transform(['remote_job01', 'remote_job02', 'remote_job03'], {
        '`$PACK`': ['', {
                '`$KEY`': '`$COPY`',
    ...
    if (live) {
        ...
        client = new __1.RemoteJobsApiRssFeedSDK(merge([
...
//# sourceMappingURL=RemoteJobEntity.test.js.map