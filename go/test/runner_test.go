...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("REMOTEJOBSAPIRSSFEED_TEST_LIVE") == "TRUE" ||
		os.Getenv("REMOTEJOBSAPIRSSFEED_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("REMOTEJOBSAPIRSSFEED_TEST_EXPLAIN"); explain != "" {
