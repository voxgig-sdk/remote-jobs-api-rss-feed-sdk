...
// RemoteJobsApiRssFeed SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class RemoteJobsApiRssFeedTestFeature extends RemoteJobsApiRssFeedBaseFeature
{
    ...
    public function init(RemoteJobsApiRssFeedContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (RemoteJobsApiRssFeedContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = RemoteJobsApiRssFeedParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (RemoteJobsApiRssFeedContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(RemoteJobsApiRssFeedContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = RemoteJobsApiRssFeedParam::call($ctx, $k);
