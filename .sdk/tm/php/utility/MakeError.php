...
class RemoteJobsApiRssFeedMakeError
{
    public static function call(?RemoteJobsApiRssFeedContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new RemoteJobsApiRssFeedContext([], null);
        }
        $op = $ctx->op ?? new RemoteJobsApiRssFeedOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new RemoteJobsApiRssFeedResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof RemoteJobsApiRssFeedError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new RemoteJobsApiRssFeedError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof RemoteJobsApiRssFeedError) {
