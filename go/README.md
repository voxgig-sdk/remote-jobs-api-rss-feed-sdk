# RemoteJobsApiRssFeed Golang SDK



The Golang SDK for the RemoteJobsApiRssFeed API — an entity-oriented client using standard Go conventions. No generics required; data flows as `map[string]any`.

It exposes the API as capitalised, semantic **Entities** — e.g. `client.RemoteJob(nil)` — each with the same small set of operations (`List`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
## Install
```bash
go get github.com/voxgig-sdk/remote-jobs-api-rss-feed-sdk/go@latest
```

The Go module proxy resolves the version from the `go/vX.Y.Z` GitHub
release tag — see [Releases](https://github.com/voxgig-sdk/remote-jobs-api-rss-feed-sdk/releases) for the available versions.
...
```bash
go mod edit -replace github.com/voxgig-sdk/remote-jobs-api-rss-feed-sdk/go=../remote-jobs-api-rss-feed-sdk/go
```
...
```go
func TestSDK(testopts map[string]any, sdkopts map[string]any) *RemoteJobsApiRssFeedSDK
```

Creates a test-mode client with mock transport. Both arguments may be `nil`.

### RemoteJobsApiRssFeedSDK methods
...
| `RemoteJob` | `(data map[string]any) RemoteJobsApiRssFeedEntity` | Create a RemoteJob entity instance. |

### Entity interface (RemoteJobsApiRssFeedEntity)

All entities implement the `RemoteJobsApiRssFeedEntity` interface.
...
| `salaryMax` | `any` | Max salary (if applicable) |
| `salaryMin` | `any` | Min salary (if applicable) |
...
```
github.com/voxgig-sdk/remote-jobs-api-rss-feed-sdk/go/
├── remote-jobs-api-rss-feed.go        # Root package — type aliases and constructors
...
The root package (`github.com/voxgig-sdk/remote-jobs-api-rss-feed-sdk/go`) re-exports everything needed
...
remotejob.List(nil, nil)

// remotejob.Data() now returns the remotejob data from the last list
// remotejob.Match() returns the last match criteria
```
