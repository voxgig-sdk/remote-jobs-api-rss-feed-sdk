-- RemoteJobsApiRssFeed SDK RemoteJob entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local RemoteJobEntity = {}
RemoteJobEntity.__index = RemoteJobEntity


function RemoteJobEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RemoteJobEntity)
  self._name = "remote_job"
  self._client = client
...
function RemoteJobEntity:get_name()
  return self._name
end


function RemoteJobEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RemoteJobEntity.new(self._client, opts)
end
...
function RemoteJobEntity:mark_deleted()
  self._deleted = true
end


function RemoteJobEntity:deleted()
  return true == self._deleted
end


function RemoteJobEntity:data_set(args)
  if args ~= nil then
...
function RemoteJobEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RemoteJobEntity:match_set(args)
  if args ~= nil then
...
function RemoteJobEntity:match_get()
...
function RemoteJobEntity:stream(action, args, callopts)
...
function RemoteJobEntity:_run_op(ctx, post_done)
  local utility = self._utility
