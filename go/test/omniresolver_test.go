...
import (
	...
	omni "github.com/voxgig-sdk/remote-jobs-api-rss-feed-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.RemoteJobsApiRssFeedError); is {
				out := map[string]any{
					"name":    "RemoteJobsApiRssFeedError",
...
func omniCtx(arg any, client *sdk.RemoteJobsApiRssFeedSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.RemoteJobsApiRssFeedSDK); is {
			client = live
