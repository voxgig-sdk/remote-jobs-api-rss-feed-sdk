...
describe("RemoteJobEntity", function()
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:RemoteJob(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:RemoteJob(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set REMOTE_JOBS_API_RSS_FEED_TEST_REMOTE_JOB_ENTID JSON to run live")
      return
    end
...
function remote_job_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("REMOTE_JOBS_API_RSS_FEED_TEST_REMOTE_JOB_ENTID")
  ...
  if env["REMOTE_JOBS_API_RSS_FEED_TEST_LIVE"] == "TRUE" then
  ...
  local live = env["REMOTE_JOBS_API_RSS_FEED_TEST_LIVE"] == "TRUE"
  return {
    client = client,
    data = entity_data,
