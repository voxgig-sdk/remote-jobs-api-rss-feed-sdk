# RemoteJobsApiRssFeed PHP SDK



The PHP SDK for the RemoteJobsApiRssFeed API — an entity-oriented client using PHP conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `$client->RemoteJob()` — with named operations (`list`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
print_r(array_map(fn($item) => $item->data_get(), $remotejob));
```
...
```php
$client = RemoteJobsApiRssFeedSDK::test($testopts, $sdkopts);
```
...
```
php/
├── remotejobsapirssfeed_sdk.php          -- Main SDK class
...
The main class (`remotejobsapirssfeed_sdk.php`) exports the SDK class
...
// $remotejob->data_get() now returns the remotejob data from the last list
