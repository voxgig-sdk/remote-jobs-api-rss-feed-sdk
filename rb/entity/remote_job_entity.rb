...
class RemoteJobEntity
  ...
  def make
    opts = @_entopts.dup
    RemoteJobEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = RemoteJobsApiRssFeedHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [RemoteJob, Hash] the current RemoteJob data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = RemoteJobsApiRssFeedHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of RemoteJob fields)
  def match_get
  ...
  #
  # @param reqmatch [RemoteJobListMatch, Hash, nil] match filter (any subset of
  #   RemoteJob fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<RemoteJob>, Array] the matching RemoteJob items; raises RemoteJobsApiRssFeedError on failure
