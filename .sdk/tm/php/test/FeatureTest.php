...
// RemoteJobsApiRssFeed SDK feature test
//
...
class FtCtrl extends RemoteJobsApiRssFeedControl
{
...
class FtClient
{
    ...
    private RemoteJobsApiRssFeedUtility $utility;

    public function __construct(RemoteJobsApiRssFeedUtility $utility, array $options)
    {
    ...
    public function get_utility(): RemoteJobsApiRssFeedUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (RemoteJobsApiRssFeedContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public RemoteJobsApiRssFeedContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (RemoteJobsApiRssFeedContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = RemoteJobsApiRssFeedConfig::make_feature($name);
    ...
    private static function buildUrl(RemoteJobsApiRssFeedSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof RemoteJobsApiRssFeedError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new RemoteJobsApiRssFeedResponse($fetched);
        }
    ...
    private function fail(RemoteJobsApiRssFeedContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (RemoteJobsApiRssFeedContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
