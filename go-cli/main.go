// Command RemoteJobsApiRssFeed-cli is a CLI + REPL for the RemoteJobsApiRssFeed SDK,
...
const prompt = "remote-jobs-api-rss-feed"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "remote_job"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      REMOTE_JOBS_API_RSS_FEED_APIKEY (api key), REMOTE_JOBS_API_RSS_FEED_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: REMOTE_JOBS_API_RSS_FEED_APIKEY carries the API key and
	// REMOTE_JOBS_API_RSS_FEED_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("REMOTE_JOBS_API_RSS_FEED_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("REMOTE_JOBS_API_RSS_FEED_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewRemoteJobsApiRssFeedSDK(opts)
