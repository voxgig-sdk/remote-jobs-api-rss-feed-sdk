...
// RemoteJobsApiRssFeed SDK retry feature
...
class RemoteJobsApiRssFeedRetryFeature extends RemoteJobsApiRssFeedBaseFeature
{
    ...
    public function init(RemoteJobsApiRssFeedContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (RemoteJobsApiRssFeedContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(RemoteJobsApiRssFeedContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(RemoteJobsApiRssFeedContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
