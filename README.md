...
Official API to retrieve the latest remote job listings from Jobicy.

Learn more about Jobicy Remote Jobs API at [jobicy.com](https://jobicy.com).
...
This SDK exposes the API as a small set of **semantic entities** — RemoteJob — that you
...
const items = await client.RemoteJob().list()
```
...
```go
client := sdk.Test()
result, err := client.RemoteJob(nil).List(
    nil, nil,
)
```
...
| --- | --- | --- |
| **RemoteJob** | The RemoteJob entity (list). | `/api/v2/remote-jobs` |
...
$remotejobs = $client->RemoteJob()->list();
print_r(array_map(fn($item) => $item->data_get(), $remotejobs));
```
