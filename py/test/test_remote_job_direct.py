...
class TestRemoteJobDirect:

    def test_should_direct_list_remote_job(self):
        setup = _remote_job_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-remote_job", "live" if setup["live"] else "unit")
        if _skip:
...
def _remote_job_direct_setup(mockres):
