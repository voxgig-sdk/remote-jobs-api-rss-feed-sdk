...
class RemoteJobDirectTest < Minitest::Test
  def test_direct_list_remote_job
    setup = remote_job_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-remote_job", setup[:live] ? "live" : "unit")
    if _should_skip
...
def remote_job_direct_setup(mockres)
  ...
  if live
    ...
    client = RemoteJobsApiRssFeedSDK.new(merged_opts)
    return {
      client: client,
      calls: calls,
      live: true,
