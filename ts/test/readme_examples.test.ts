...
//     (\`new RemoteJobsApiRssFeedSDK(...)\` / \`RemoteJobsApiRssFeedSDK.test(...)\` become
//     \`RemoteJobsApiRssFeedSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'RemoteJobsApiRssFeedSDK'
...
const TEST_SEED = {"entity":{"remote_job":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new RemoteJobsApiRssFeedSDK(...)\` and any
// \`RemoteJobsApiRssFeedSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+RemoteJobsApiRssFeedSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/RemoteJobsApiRssFeedSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
