...
//    only - a typed RemoteJobsApiRssFeedContext there would make every ctx
...
final class RemoteJobsApiRssFeedOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new RemoteJobsApiRssFeedContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new RemoteJobsApiRssFeedSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new RemoteJobsApiRssFeedResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new RemoteJobsApiRssFeedError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new RemoteJobsApiRssFeedResponse($clean['response']);
    ...
    public static function err_from_map($m): ?RemoteJobsApiRssFeedError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new RemoteJobsApiRssFeedError(is_string($code) ? $code : '', $msg);
    }
