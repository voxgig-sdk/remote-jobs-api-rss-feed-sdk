...
from remotejobsapirssfeed_sdk.remotejobsapirssfeed_types import (
    RemoteJob,
    RemoteJobListMatch,
)
...
class RemoteJobEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return RemoteJobEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[RemoteJob]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.RemoteJob().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
