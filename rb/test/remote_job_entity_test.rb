...
class RemoteJobEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = RemoteJobsApiRssFeedSDK.test(seed, nil)
    seen = base.RemoteJob(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.RemoteJob(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set REMOTE_JOBS_API_RSS_FEED_TEST_REMOTE_JOB_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def remote_job_basic_setup(extra)
  ...
  entid_env_raw = ENV["REMOTE_JOBS_API_RSS_FEED_TEST_REMOTE_JOB_ENTID"]
