# remote-jobs-api-rss-feed-cli

boru-driven command-line client **and** interactive REPL for the RemoteJobsApiRssFeed
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/remote-jobs-api-rss-feed-cli)
make build

# 2. See usage (words, entities, env vars)
./remote-jobs-api-rss-feed-cli --help

# 3. Provide credentials once, via the environment
export REMOTE_JOBS_API_RSS_FEED_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./remote-jobs-api-rss-feed-cli list remote_job

# 5. Override the API base URL for a single call
REMOTE_JOBS_API_RSS_FEED_BASE=https://api.example.com ./remote-jobs-api-rss-feed-cli list remote_job

# 6. No arguments -> interactive REPL
./remote-jobs-api-rss-feed-cli
remote-jobs-api-rss-feed> list remote_job
remote-jobs-api-rss-feed> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/remote-jobs-api-rss-feed-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/remote-jobs-api-rss-feed-cli list remote_job
   ```
...
Run with no arguments to open a REPL (prompt `remote-jobs-api-rss-feed>`). Each line is
evaluated as its own boru expression:

```text
$ ./remote-jobs-api-rss-feed-cli
remote-jobs-api-rss-feed> list remote_job
remote-jobs-api-rss-feed> /help
remote-jobs-api-rss-feed> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `remote_job`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/remote-jobs-api-rss-feed-cli`. |
...
(`./remote-jobs-api-rss-feed-cli <expr>`) and interactively (the REPL), and expressions compose the
